...
| **Le parentele fra le case (ADR-092)** — decisione del proprietario (2026-08-18), nata da una domanda sulla visione («manda una foto al gosino di nonno Sandro») la cui analisi sta nel BACKLOG gruppo 21: riapre **parzialmente** ADR-019/020 e ne conferma il resto. Due case si legano con **consenso bilaterale** (`household_ties`: proposta → accettata, revoca unilaterale; l'avvertenza «diventa possibile inviare messaggi e ricordi; ogni invio parte solo da una tua azione esplicita» sta nel markup del pannello PRIMA dei click), e da lì viaggiano le **cartoline** (`parcels`): messaggio o ricordo, testo, un elemento per volta. **`ParcelService.send()` è l'unico punto d'invio** — mai iniziativa, sogno o job — e la guardia `parcelGate.test.ts` è strutturale, non lessicale: fuori dai file autorizzati **nominare** il servizio è già l'infrazione (la prima stesura cercava `parcels.send(` e un rinomino la disarmava: verificata rossa con l'infrazione apposta, la lezione delle guardie di ADR-091). Il testo a riposo è **ri-cifrato con la DEK della casa DESTINATARIA** (provato: il mittente non riapre ciò che ha spedito); RLS **bilaterale** sul precedente di `adoptions` (migrazione 0049 a mano: coppia unica in qualunque verso con indice funzionale parziale — una revocata non blocca il ripensamento — INSERT solo come mittente e solo `proposta`, UPDATE solo del destinatario, `GRANT UPDATE` **per colonna**: il testo non si riscrive nemmeno da destinatari, DELETE revocato). La consegna è un **desiderio del gosino destinatario** («è arrivata una cartolina…», pattern ADR-078) — mai la voce di un'altra casa dentro casa tua nel momento deciso da loro; un ricordo si «tiene» e diventa `memories` **in chiaro** (ADR-091) con l'origine dichiarata nel testo. Gesto a voce «manda ai nonni: …» (forma chiusa coi due punti obbligatori, famiglia ADR-063/065; il confronto dei nomi spoglia gli articoli — «manda AI nonni» deve trovare «i nonni»), esiti distinti: nessuna parentela ≠ proposta in attesa ≠ partita. **Export (GDPR)**: la guardia di copertura di ADR-089 ha preteso la decisione — le ricevute escono in chiaro con la DEK di casa, delle spedite esce **la busta** (un export che riaprisse il testo smentirebbe la promessa dell'invio). Perimetro dichiarato: stessa installazione, solo testo (la foto dipende dall'album, gruppo 21). Giro regola 12: **BO** schema+RLS, TieService/ParcelService, rotte `/v1/ties` `/v1/parcels`, gesto in `ChatService`, export, 4 verbi d'audit; **`/admin`** pagina «Le parentele» (avvertenza nel markup con `data-testid`, proporre/accettare/revocare, cassetta della posta col «tienilo», invio che chiede **quale** esemplare — mai il default), `call()` ovunque e id verificati da `script.test.ts`; **FE** nessuna modifica e non serviva: la consegna parla dalla porta dei desideri esistente, nessun contratto WS nuovo, **nessun bundle da ricostruire**; **`ops/jobs`** nessuna modifica (il backup scopre le tabelle dallo schema, e le bilaterali entrano nel backup di entrambe — dichiarato in ADR-092). Test: 11 unit parser + 3 guardia + 11 RLS (db) + 15 d'integrazione soul (fra cui: zero righe su `budget_ledger`, il provider è una porta chiusa, il vicino non vede niente nemmeno con l'id in mano). **Nota di sessione**: sandbox senza Docker — `startPostgres()` ha guadagnato il ripiego dichiarato `UGO_TEST_PG_URL` (Postgres 16+pgvector locale, un database usa-e-getta per chiamata; senza la variabile, Testcontainers come sempre), i due test di `@ugo/db` che istanziavano il container a mano sono passati alla factory, e con quello **l'intera suite qui eseguibile è verde** (soul integrazione: 322 passati, 0 falliti); le ~20 suite che istanziano ancora `PostgreSqlContainer` direttamente restano alla CI in questa sandbox, come quelle che vogliono Ollama — convertirle alla factory è un lavoro suo, non un Boy Scout di passaggio | I nonni non sono «i vicini»: una busta con mittente e francobollo non è una federazione |
