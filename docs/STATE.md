...
| **Whisper metteva in bocca al proprietario i titoli di coda, e il muso taceva mentre trascriveva** — dal registro del chiosco (2026-08-21): `tu: Sottotitoli e revisione a cura di QTSS`. Nessuno l'ha detto: è l'**allucinazione su silenzio** più nota di whisper, addestrato su montagne di video sottotitolati. Ci arriva perché il cancello degli enunciati è un misuratore di **livello**, non un rilevatore di parlato: una porta o un colpo di tosse fanno il salto di 9 dB che lo apre. E non era un fastidio: quella frase diventava un turno e **veniva scritta in biografia come detta dal proprietario** — lo stesso difetto peggiore che ADR-110 aveva appena chiuso dall'altro lato, con il volto che indovinava l'autore; qui è l'orecchio che inventa il testo. **Whisper ce lo dice e buttavamo via il numero**: ogni segmento porta `no_speech_prob` e `avg_logprob`, e sia la percezione sia l'ingest tenevano `segment.text` scartando entrambi. Ora `whisper_noise` (puro, condiviso) filtra con i numeri — la difesa vera, che vale anche per le allucinazioni mai viste — più una cintura di code note; e sta in **tutti e due** i percorsi, perché in `ingest.py` quel testo non diventa una frase in chat ma un `transcript_segment` con un embedding sopra, ripescabile per sempre. Taratura dichiarata: **meglio lasciar passare una boilerplate che mangiarsi una frase vera** — una frase persa è il difetto che abbiamo appena inseguito per due giorni. Seconda metà, dalla stessa segnalazione: «deve darmi un'indicazione che ha sentito e sta pensando, altrimenti non si capisce se è arrivata la voce». Il buco era largo — con la dettatura di casa il muso restava fermo da quando smetti di parlare a quando whisper risponde, e quel silenzio è indistinguibile dall'essere sordo. Ora **quattro tempi**: `ascolto` mentre parli (il cancello che si apre, l'unico che dice «la voce è arrivata» invece di «forse»), `pensa` appena il clip parte, `pensa` mentre soul lavora, `parla`. E `handleHeardText` non torna più a `listening`: era un passo all'indietro sullo schermo proprio mentre si metteva a rispondere | Un silenzio indistinguibile dalla sordità è un guasto, anche quando sotto funziona tutto |
| **«Fallo sognare adesso» non faceva niente, e lo diceva in un rapporto pieno di `skipped`** — l'ultimo anello della caccia di due giorni, trovato dal rapporto del sogno mandato dal proprietario: `"enroll": "skipped (already done)"` con **due voci in coda**. I marcatori del sogno sono per **data + passo + esemplare**, quindi il sogno delle 02:30 marca `enroll` come fatto per oggi e **ogni sogno successivo dello stesso giorno lo salta** — comprese le voci arruolate nel frattempo. Il pannello intanto prometteva «l'impronta nasce stanotte, **o subito se premi "Fallo sognare adesso"»**, e quella promessa era **falsa per ventuno ore su ventiquattro**. Il rimedio non è nuovo, è quello che il `backup` fa da sempre nello stesso `if`: **un marcatore che può essere stantìo non si crede sulla parola, si verifica contro la realtà** — per il backup è «l'oggetto c'è ancora nel bucket», per l'arruolamento è «la coda è vuota». Rieseguire è sicuro perché `run_enroll` è idempotente per costruzione (`_pending` esclude le richieste già lavorate), ed è la ragione per cui il marcatore lì non comprava niente e costava questo. Nello stesso giro, la legibilità: il proprietario ha guardato la riga «a metà servizio» e ha chiesto **«ma WHY?»** — che è la prova che la riga aveva fallito il proprio metro. Adesso la causa porta la croce come i mestieri della percezione (`✗ 2 voci in attesa`): il segno che fa scattare l'etichetta si vede, invece di doverlo dedurre. Test: 4 pytest nuovi su Postgres vero (coda vuota, richiesta senza esito, richiesta già lavorata, e la coda di un'altra casa che non è la mia — ADR-019) | Un marcatore che può essere stantìo non si crede sulla parola |
| **«La voce non la riconosce MAI»: era in coda, e la coda non aveva un numero** — chiusa la caccia di due giorni. `storeVoiceSample` restituisce **`queued`**: arruolare una voce non crea l'impronta, deposita un clip e scrive una richiesta, e l'impronta la costruisce il **sogno** (`run_enroll`). Il proprietario aveva provato tutte e due le strade e nessuna aveva mai chiuso il giro: dal chiosco il clip sforava il contratto e non entrava nemmeno in coda; dal pannello entrava, ma serviva un sogno **e** i job cablati sulla percezione. **Verifica prima di scrivere**: entrambe le superfici il messaggio ce l'avevano già («Grunf, ti ho sentito! Stanotte imparo la tua voce», «Me lo segno. L'impronta nasce stanotte») — quindi il rimedio che avevo proposto (dirlo) era **già fatto**, e l'ho scoperto guardando invece di costruirlo. Il buco vero era un altro: **una frase detta una volta non è uno stato**. Il giorno dopo non c'era nessun posto dove leggere se quella richiesta fosse stata lavorata, restasse in coda, o non fosse mai arrivata — e «fallo sognare adesso» era un gesto scaramantico invece di una prova. Ora la riga **Il sogno notturno** dice `ultimo sogno 8 ore fa · 1 voce in attesa`, con stato **a metà servizio** e il consiglio: falle nascere e **il numero deve tornare a zero**. La query è **la stessa** di `enroll_step._pending`, apposta: se divergessero, il pannello direbbe una cosa e la notte ne farebbe un'altra. Boy Scout sul file toccato: `catalogue.ts` era a 286 righe (regola 10) e le due sonde che **leggono** la risposta invece di riceverla — percezione e sogno — sono uscite in `readings.ts`, che è anche la cosa che hanno in comune: nascono tutt'e due dallo stesso difetto, un `200` che non dice niente letto come se dicesse tutto | Una frase detta una volta non è uno stato |
| **ADR-110 corretta il giorno stesso: il volto dice chi c'è, la voce dice chi parla** — il proprietario, guardandola funzionare: «il volto deve dire chi è PRESENTE, ma è la voce che dice chi sta PARLANDO. Altrimenti se io sono presente e qualcuno mi chiama, attribuisce a me la chiamata». **Il difetto era più profondo della guardia sui due volti** che avevo messo: quella copre due presenti inquadrati, ma da UN solo volto in stanza non segue che sia lui a parlare — basta qualcuno fuori inquadratura, una voce dal corridoio, la televisione. E la conseguenza non è una risposta imprecisa: è una frase **attribuita a chi non l'ha detta**, scritta in biografia col suo nome sopra e ripescata per sempre. L'errore di progetto, per intero: avevo trattato «chi c'è» e «chi parla» come la stessa domanda con due fonti di qualità diversa, mettendo la peggiore come ripiego della migliore. Sono due domande. Ora `beingId` torna a venire **solo dalla voce**, nasce `presentBeingIds` sul contratto, `RecentFaces.only()` diventa `all()`, e il blocco del branco riceve chi parla **e** chi c'è — il prompt aveva già la casella giusta («**Chi c'è adesso**») e `unidentifiedPresent` resta vero quando la voce non riconosce nessuno: vedere Francesco non autorizza a dedurre che parli Francesco. L'ADR **non è stata rinominata**, apposta: il titolo sbagliato resta leggibile sopra la correzione. Runbook corretto nello stesso giro (§2-ter punto 3 diceva «il nome della risorsa È l'hostname», vero per i Service e **falso per le Application**, i cui container cambiano nome a ogni deploy — due guasti sono nati da lì) | «Chi c'è» e «chi parla» non sono la stessa domanda con due fonti di qualità diversa |
| **Il volto concorre a dire chi parla (ADR-110)** — direttiva del proprietario, dopo aver fatto tutto il giro per insegnargli la propria faccia, averla vista in «Facce che conosce», e essersi sentito chiedere «chi sei tu?» due minuti dopo: «deve riconoscere chi parla, assolutamente. Solo se ha marchiato non riconoscere non deve farlo». Non era un guasto — era il progetto: l'identità di un turno veniva da **una riga sola** (`byVoice`), e l'esito di `byFace` veniva **buttato** dopo aver deciso «questa faccia la conosco, non chiedere chi è». Ora `who = byVoice ?? faces.only(at)`, con tre proprietà e la terza è quella che rende la cosa accettabile: (1) **la voce vince sempre** — è l'unica che dice chi ha *parlato*; (2) **novanta secondi**, tre frame della camera del chiosco: oltre, «era in casa poco fa» non è «sta parlando adesso» e diventerebbe memoria travestita da percezione; (3) **due volti diversi nella finestra ⇒ nessuna risposta**, perché attribuire ogni frase all'ultima faccia inquadrata non è riconoscere, è affermare a caso — e sbagliare il nome di chi ti sta parlando è peggio che non dirlo. `RecentFaces` è **per gateway, cioè per esemplare**: due creature nella stessa stanza guardano lo stesso volto ma sono due corpi. **Le protezioni non si ricontrollano, ed è deliberato**: chi ha `no_vision`/`is_minor` non ha un'impronta del volto (l'arruolamento la rifiuta con 403 e distrugge lo stesso l'impronta ignota), quindi non può essere l'esito di `byFace` — regola 9, a monte e non a valle; ricontrollare qui vorrebbe dire due verità sullo stesso vincolo, e il giorno che divergono vince quella sbagliata. **Un effetto nuovo è scritto invece che scoperto**: chi ha `no_audio` ma ha lasciato arruolare il volto adesso può essere riconosciuto mentre parla, e non è una violazione — `no_audio` dice «non costruire un'impronta della mia voce», non «non sapere che sono io». Niente di nuovo sul database: la finestra è id e millisecondi in memoria, non sopravvive a un riavvio, e **non ha un `forget()`** perché si svuota da sola — un metodo che nessuno chiama sarebbe una promessa scritta invece di una garanzia | Sbagliare il nome di chi ti sta parlando è peggio che non dirlo |
| **Le orecchie di casa diventano la base (ADR-109)** — direttiva del proprietario: «`stt=locale` non devo impostarlo, deve essere LA BASE!». Ha ragione, e non è una preferenza: sopra `startListening` in `main.ts` c'era scritto da mesi che *«il riconoscitore del browser è quello di Google, quindi ciò che DICI esce di casa finché questo è acceso»* — dichiarato, e **acceso di default su ogni dispositivo**. Una minimizzazione che vale per tutto tranne il canale principale non è una postura, è una decorazione. La decisione ha **due metà, e la prima è prerequisito della seconda**. (1) **I due «no» si distinguono in tutta la catena**: `SttOutcome` = `text` · `unusable` · `down`; percezione risponde 422 «troppo corto» sotto gli 0,8 s — cioè a un «sì» — e `recognitionClient.transcribe` collassava OGNI esito non-200 in `undefined`, che `/v1/stt` traduceva in 503. Quindi **la correzione lato muso del giro precedente era metà correzione**: il 422 non arrivava mai al muso, arrivava un 503. Adesso 4xx ⇒ 422 (si perde il clip) e 5xx ⇒ 503 (si cambia strada). (2) **`EarsChoice.first()` parte da `locale`**, e la memoria del dispositivo smette di essere un sì/no per tenere **quale strada** — `locale` quando il browser si arrende, `browser` quando casa non c'è. `?stt=` resta la via d'uscita diagnostica in **entrambi i versi** e dimentica il ricordo: è così che si riprova una strada dichiarata morta dopo che la causa è stata rimossa. **Il prezzo è dichiarato nell'ADR**: in una casa senza whisper il primo enunciato di ogni dispositivo si perde — una volta per dispositivo e non a ogni ricarica, ed è esattamente per questo che il ripiego adesso si ricorda. Scartata l'alternativa di interrogare il server all'avvio: una rotta non guardata in più solo per dichiarare una configurazione, contro un costo che si paga una volta e si spiega in una riga | Una funzione che protegge la privacy e si accende scrivendo un parametro nell'URL è protetta per chi legge la documentazione, cioè per nessuno |
| **Verde in tre pagine, e whisper morto in mezzo** — dal campo (2026-08-20): «⚠ il corpo: whisper non risponde: torno al riconoscitore del browser», mentre `/v1/capabilities` diceva «Dettatura locale ✅» e la diagnostica diceva «percezione · risponde · 10 ms». Tutte e tre le pagine dicevano il vero e insieme mentivano, perché **guardavano cose diverse**: la capability guarda che la variabile ci sia, la mia sonda guardava che il container rispondesse `200`. Il paradosso è che `/health` di percezione dichiara i suoi cinque mestieri (`voice` `face` `stt` `tts` `ocr`) e la sua docstring dice testualmente «cosa sa fare davvero, invece di un 200 che non dice niente» — e la sonda lo riduceva a un 200. Ora la sonda **legge il corpo**, la riga mostra `voce ✓ · volto ✓ · dettatura ✗ · Piper ✓ · OCR ✓`, e nasce il quinto stato **`partial` — «a metà servizio»**: il container risponde e un mestiere dentro è a terra, che non è né «lento» né «non risponde» e finora non aveva parola. `verdictOf` lo degrada come il lento, e — cambiamento voluto — un **vitale** a metà servizio resta `degraded` e non `unavailable`: sta rispondendo, e dirlo spento manda a cercare il guasto sbagliato. **E la causa vera del sintomo era un'altra ancora**: percezione risponde **422 «troppo corto»** a un clip sotto gli 0,8 s — cioè a un «sì» — e il muso contava *qualunque* codice non-ok fra i fallimenti, quindi **tre monosillabi di fila dichiaravano whisper morto**; la scelta veniva poi RICORDATA fra le ricariche (`earsChoice`), quindi la strada di casa non tornava più da sola. Adesso solo 5xx e rete assente dicono «sono giù»; un 4xx costa il clip, non la strada. Terzo difetto sulla stessa strada: il caricamento di whisper stava in un `except Exception: self.stt = None` **senza un log**, quindi non si poteva distinguere «modello non scaricato» da «modello inesistente» da «disco pieno» | Tre pagine verdi che guardano tre cose diverse non fanno una diagnosi |
| **L'arruolamento della voce si rifiutava da solo, e col volto non c'entrava niente** — dal campo (2026-08-20): «⚠ il corpo: la voce non è partita: il clip è troppo lungo per il contratto», e mezz'ora dopo «ha smesso di riconoscermi, in faccia e in voce». **Un difetto solo, due sintomi.** Il tetto di `voice_sample` (120 000 caratteri) era giustificato da un'aritmetica scritta in un commento — «dieci secondi di webm/opus a ~2-4 kB/s» — che descriveva un `MediaRecorder` che **nessuno aveva configurato**: senza `audioBitsPerSecond` il ritmo lo sceglie il browser, Chrome lo sceglie alto, e dieci secondi sforano. Risultato peggiore possibile: si chiede alla persona di parlare per dieci secondi, e POI si butta via il clip. Ora il ritmo è **dichiarato** (`VOICE_SAMPLE_BITRATE = 24_000` nel contratto, cioè dove sta il tetto che giudica) e copiato nel pannello, con **due test**: l'aritmetica del tetto (10 s ⇒ 40 000 caratteri, sotto la metà del tetto — un pareggio lo romperebbe il prossimo browser) e la **guardia contro la deriva** fra i due lati che registrano la stessa cosa (il pannello non può importare: è una stringa servita, quindi copia — e il test è ciò che impedisce alla copia di andare per conto suo). Secondo difetto sulla stessa strada: un rifiuto chiamava `dismiss()`, quindi il bottone spariva mentre `frameFor` **non** aveva consumato l'invito — l'invito sopravviveva invisibile e non c'era più niente da premere. Un rifiuto costa un tentativo, non la strada. **E il sintomo del riconoscimento non era un guasto**: in una conversazione chi sei lo dice **solo la voce** (`faceGateway` §341), il volto serve unicamente a decidere se conservare un'impronta ignota e chiederti chi è — quindi senza impronta vocale non c'è identità, per quanto bene la faccia sia conosciuta. Il sogno era un falso colpevole: la scadenza delle impronte ignote tocca solo quelle oltre i 30 giorni, e il fallimento delle 11:02 precede il sogno ed è già sufficiente | Un tetto giustificato da un'aritmetica che nessuno esegue è una scommessa sul browser di turno |
...
| **L'astensione: il criterio relativo non regge (ADR-106)** — gruppo 1, e **è un risultato negativo**. ADR-022 aveva concluso «serve un criterio relativo» e la conclusione era rimasta lì con l'aria della cosa ovvia. Misurata prima di scriverla: `gap`, `gapRel` e `plateau` non separano male, separano **al contrario** — «quando è il compleanno della nonna?», che una risposta ce l'ha, ha `plateau` −0.0504, più negativo di tutte e tre le domande senza risposta; un criterio su quei segnali si asterrebbe da una risposta che esiste. La ragione col senno della misura è semplice: misurano quanto il primo **stacca gli altri**, e un ricordo giusto ma isolato non stacca niente. `bothArms`/`lexHits` non sono segnali di astensione ma **positivi a senso unico** (zero falsi positivi su 6, ma solo 6 delle 10 con risposta ce l'hanno vero). `top1` separava per **28 decimillesimi**: la stessa trappola dei 0.675, altro numero — e rimisurato sui dieci negativi **non separa più**, bande sovrapposte di quattro centesimi con tre domande senza risposta sopra la più bassa con risposta. Quei 28 decimillesimi erano un artefatto dei tre negativi facili, ed è la prova che allargare il corpus non era diligenza ma la correzione di una misura sbagliata. **E il banco non poteva accorgersene**: tre negativi soli, tutti lontani dal dominio, quindi nessuno poteva falsificare `bothArms` — un segnale che nessun caso può smentire non è stato misurato, è stato assunto. Corpus da 3 a **10** negativi con sette **near-miss** dentro il dominio (il cane del vicino che c'è e non ha nome, il colore della macchina di cui c'è la targa, **la password del wifi Cinghiale** che aggancia il braccio lessicale su un ricordo dove la password non c'è). Nessun meccanismo messo in produzione, e il floor resta 0: una formula che il banco smentisce sarebbe peggio del nulla, perché darebbe l'impressione che l'astensione sia risolta. `bothArms` invece ha retto la prova dura: **zero falsi positivi su dieci** negativi, near-miss compresi — nemmeno la password del wifi Cinghiale accende il braccio lessicale — quindi come indizio POSITIVO è misurato dove poteva rompersi, pur restando muto sul «no». Prossima strada scritta: **verifica del modello locale** sui ricordi ripescati (mai il provider, regola 3), adesso misurabile su dieci negativi veri | Un segnale che nessun caso può smentire non è stato misurato: è stato assunto |
| **Il genoma si rilegge (ADR-105)** — ultima voce del **gruppo 14**, che con questa è **chiuso a zero**. `trait_sets` si vedeva per un istante nel modulo della nascita e poi spariva: com'è fatto un gosino si sapeva solo aprendo psql e sapendo leggere un jsonb di alleli. Due domande restavano senza risposta, e la seconda è quella che conta: «perché è fatto così» (il pannello mostrava la *persona*, che è una frase) e «**cosa passerà ai figli**». Da ADR-068 c'è dominanza — `spots` recessivo, `boldness` dominante — quindi **un allele coperto non si vede addosso e passa lo stesso**: è il motivo per cui da due genitori senza chiazze nasce ogni tanto un cucciolo a chiazze, un fatto che il motore calcolava da mesi e che nessuno poteva guardare (chi allevava vedeva un risultato inspiegabile e doveva fidarsi). `GET /v1/gosini/:id/genome` **in sola lettura** torna per ogni gene le due copie, come si esprime, il valore espresso e — quando c'è — quello che porta senza mostrarlo; più ceppo, versione (e quante ne esistono) e la nota di provenienza («cucciolata seed=… cucciolo=…»). `hiddenAllele` sta in `packages/psyche` accanto a `expressGene` e non nella rotta: duplicare le regole dell'espressione in un handler HTTP vuol dire che un giorno le due copie diranno cose diverse. Soglia `HIDDEN_ALLELE_GAP = 0.15` dichiarata e testata su **entrambi** i lati. **Nessuna scrittura, e non ci sarà** (regola 13): era tutto già scritto, mancava il vetro — zero migrazioni. Giro regola 12: **BO** motore + rotta; **`/admin`** sezione «Com'è fatto» sotto «Da chi discende» (le due domande sono la stessa guardata da due lati); **FE** nessuna modifica e non serviva — e in vetrina resta la regola di ADR-083: aspetto e pedigree, **mai il temperamento in numeri**, perché chi compra guarda una creatura e non una scheda tecnica. Test: 3 d'integrazione (fondatore omozigote che non porta niente, nato che porta e lo dice, vicino a 404) + 5 unit sulle tre espressioni e sulla soglia | Un motore che calcola una cosa vera e non la mostra trasforma l'allevamento in una scatola nera |
| **La scrivania (ADR-104)** — chiude il **gruppo 14**: le azioni che si facevano solo in psql. Rilettura contro il codice prima di scrivere una riga: tre voci su nove erano già chiuse (stanza da ADR-100, budget dal pannello, `PATCH /v1/account`), e una non era una mancanza ma un **difetto**. **L'interruttore dell'iniziativa** viveva in un campo in RAM **per processo**: si spegneva dal pannello e tornava acceso al riavvio, e — da quando le case sono più di una — lo spegneva **a TUTTE**, col vicino che smetteva di ricevere una parola senza che nessuno potesse sapere perché. Ora `accounts.initiative` nullable con **tre** stati (`null` = non deciso qui, decide `UGO_INITIATIVE`), `initiativeEnabled` da `() => boolean` a `(accountId) => boolean`, e il pannello legge **la riga e non la cache** (la mappa in memoria serve solo al tick sincrono; una query su una pagina aperta a mano è il prezzo giusto per non mentire mai). Supera ADR-034 dichiaratamente: la sua preoccupazione — un silenzio che dura senza che nessuno ricordi perché — non si risolve dimenticando la scelta ma **facendola vedere**, e infatti c'è il gesto per restituire la parola al server. Poi: **ritiro** reversibile (`retired_at` c'era dal 2026 e lo scriveva solo una UPDATE a mano; la rotta ricarica il registro, o il ritirato risponderebbe fino al riavvio, e `GET /v1/gosini` porta `retiredAt` perché un pannello che mostra attivo chi non risponde è peggio di uno muto); **ricordo scritto a mano** (`archive.ts` sapeva correggere e cancellare, non scrivere — in chiaro per ADR-091, vettore calcolato FUORI dalla transazione che ha dichiarato la casa, e `embedded:false` detto invece che scoperto mesi dopo); **desideri** dati e annullati (`expired`, non cancellati: una sveglia sparita non spiegherebbe più perché quella sera non ha detto niente); **riunioni** rilette e buttate — in un modulo loro, registrate **anche senza Vexa** (chi ha staccato l'integrazione non deve restare con trascrizioni che non può né leggere né cancellare) e con cancellazione **vera**, perché una trascrizione non è biografia di UGO ma le parole di chi ha parlato; **scadenza impronte** a richiesta. Quattro verbi nuovi sul giornale, i desideri no (un registro che segna tutto è un registro che nessuno legge). Giro regola 12: **BO** migrazione 0053, switch, cinque rotte, due moduli nuovi; **`/admin`** ritiro sulla pagina «stato», «Diglielo tu» sulla memoria, aggiungi/annulla sui desideri, rileggi/butta sulle riunioni, scadenza sulle facce, e il terzo stato dell'interruttore che prima era irraggiungibile; **FE** nessuna modifica e non serviva — sono gesti del titolare dal pannello, il muso non li vede e `faceContracts` non cambia. Test: 13 d'integrazione + censimento RLS da 25 a **31**, col morso verificato togliendo la transazione alla riga di giornale (rosso) | Un gesto che si fa solo in psql, in un progetto che si dichiara adottabile, è un gesto che non si può fare |
| **La cucciolata (ADR-103)** — direttiva del proprietario: *«dai 2 agli 8, casuale, non sceglibile […] i nomi si possono scegliere, non devi tenerne uno solo, anzi, la cucciolata ti aggiunge tutti i gosini […] due genitori non possono avere altre cucciolate per un mese»*. Tre cose insieme. **La taglia è un dado**: `drawLitterSize` (2..8 uniforme, 1 e 10 al 2% ciascuna) letto UNA volta e PRIMA dei genomi, sullo stesso flusso di `mulberry32` — così il seme determina la cucciolata intera, numero compreso, e l'anteprima resta aritmetica. `litterSize` **sparisce dal contratto HTTP** e resta solo nel motore puro (i test statistici vogliono 400 cuccioli per misurare un tasso di mutazione): la manopola non esiste dove sta il proprietario. **Nascono tutti**: `names[]` allineati all'anteprima invece di `cubIndex`+`name`, i bocciati dallo screening tornano in `stillborn` col motivo — un no detto, non un fratello scomparso; la selezione si sposta a posteriori (si guarda crescere chi c'è, e chi non diventa il tuo si cede o si ritira). **Costa**, dalla terza generazione in poi (`generazione figlio ≥ 2`; i figli dei capostipiti gratis — far pagare il primo passo sarebbe far pagare l'inizio di una stirpe), `UGO_LITTER_COST_USD` **per cucciolo** (default 0,25 $), dal **salvadanaio dei genitori** in parti uguali, e **anche a noi**: l'allevamento fondatore non ha sconto. Riga su `budget_ledger` con `provider = ugo` e non su una tabella nuova, perché **è lì che il salvadanaio guarda** — conseguenza accettata e dichiarata: la nascita pesa anche sul tetto giornaliero, far nascere e parlare vengono dalla stessa tasca. Muro solo col **metabolismo acceso** (come la fame): 402 con chi è a corto e di quanto; a metabolismo spento la riga si scrive lo stesso — spegnerlo nasconde il conto, non lo cancella. **Riposo di 30 giorni per la coppia esatta** (A+B ≠ A+B+C: doppio conto nella query, altrimenti aggiungere un terzo genitore faceva ripartire il cronometro), 409 con la data di libertà **anche sull'anteprima** — mostrare sei cuccioli e poi rifiutarli è il modo peggiore di dire di no. Rotta passata da orchestratore a traduttore HTTP: `litterService` (la nascita), `litterBirth` (il singolo parto), `litterCost` (conto e riposo) — regola 10. Giro regola 12: **BO** motore, tre servizi, rotta, env; **`/admin`** riscritto il blocco cucciolata (via «nome del cucciolo scelto» e «Adotta», dentro un campo nome per scheda e «Falli nascere», col conto e il riposo detti in italiano); **FE** nessuna modifica e non serviva — la cucciolata è un atto d'allevamento, il muso non la vede e `faceContracts` non la nomina. Test: 21 d'integrazione su Postgres vero (tre case, coppie dedicate perché il riposo lo impone) + 2 unit sulla forma della distribuzione su 20 000 tiri | Una manopola sul numero di figli è la stessa manopola che la regola 13 vieta sul carattere, solo su un tratto diverso |
| **Le parentele fra le case (ADR-099)** — decisione del proprietario (2026-08-18), nata da una domanda sulla visione «manda una foto al gosino di nonno Sandro» (l'analisi sta nel BACKLOG gruppo 22): riapre **parzialmente** ADR-019/020 e ne conferma il resto. Due case si legano con **consenso bilaterale** (`account_ties`: proposta → accettata, revoca unilaterale e silenziosa; l'avvertenza «diventa possibile inviare messaggi e ricordi; ogni invio parte solo da una tua azione esplicita» sta nel markup del pannello PRIMA dei click), e da lì viaggiano le **cartoline** (`parcels`): messaggio o ricordo, testo, un elemento per volta. **`ParcelService.send()` è l'unico punto d'invio** — mai iniziativa, sogno o job — e la guardia `parcelGate.test.ts` è strutturale invece che lessicale: fuori dai file autorizzati **nominare** il servizio è già l'infrazione (la prima stesura cercava `parcels.send(` e un rinomino la disarmava; verificata rossa con l'infrazione apposta, la lezione delle guardie di ADR-091). Il testo a riposo è **ri-cifrato con la DEK della casa DESTINATARIA** — provato che il mittente non riapre ciò che ha spedito; la consegna è un **desiderio del gosino destinatario** (pattern ADR-078), mai la voce di un'altra casa dentro casa tua nel momento deciso da loro; un ricordo si «tiene» e diventa `memories` **in chiaro** (ADR-091) con l'origine dichiarata. Gesto a voce «manda ai nonni: …» (forma chiusa coi due punti obbligatori, famiglia ADR-063/065, zero token), esiti distinti: nessuna parentela ≠ proposta in attesa ≠ partita. **E il merge con la Fase C ha trovato il difetto vero, che nessun test avrebbe visto.** Il ramo era verde su un main di venti commit prima; nel frattempo `households` è diventato `accounts` (ADR-092), RLS si è accesa davvero (`DATABASE_URL_APP`, ADR-062 tempo 2b) e sono nati `ugo_market`/`withMarket` (ADR-097). Sotto `ugo_app` **tutte le letture che attraversano il confine tornavano vuote, senza un errore**: lo slug della casa a cui si propone, la sua chiave per cifrare la cartolina, il suo esemplare destinatario. Le parentele sono la **quinta superficie cross-account** che ADR-097 elencava senza coprirla — e la trappola era che i test d'integrazione si collegano come proprietario delle tabelle, quindi **sarebbero rimasti verdi mentre la produzione taceva**: ADR-045 di nuovo, in un posto nuovo. Risposta: **`ugo_post` + `withPost`**, stesso meccanismo di ADR-097 (ruolo NOLOGIN, `SET LOCAL ROLE` in un punto solo, `WITH INHERIT FALSE` perché si assuma e non si porti addosso), con grants **per colonna** su `accounts` — `wrapped_data_key` compreso, ed è la riga più delicata del file: è la chiave *avvolta*, quindi il ruolo consegna una busta chiusa, non un segreto. Fuori dalla sua portata per costruzione: clienti, trascrizioni, biometria, salvadanaio, audit. E il test che mancava adesso c'è: `tiesUnderRls.integration.test.ts` si collega **come `ugo_app`** e cammina il giro intero — **verificato rosso** togliendo `withPost` (5 test su 6, «casa-sconosciuta» al primo passo), verde rimettendolo. **Export (GDPR)**: la guardia di copertura di ADR-089 ha preteso la decisione — le ricevute escono in chiaro con la DEK di casa, delle spedite esce **la busta** (un export che riaprisse il testo smentirebbe la promessa fatta all'invio). Perimetro dichiarato: stessa installazione, solo testo (la foto dipende dall'album, gruppo 22). Giro regola 12: **BO** schema + due migrazioni (0051 generata, 0052 a mano col ruolo), `withPost` in `packages/db`, TieService/ParcelService, rotte `/v1/ties` e `/v1/parcels`, gesto in `ChatService`, export, 4 verbi d'audit; **`/admin`** pagina «Le parentele» (avvertenza nel markup con `data-testid`, proponi/accetta/revoca, cassetta della posta col «tienilo», invio che chiede **quale** esemplare — mai il default), `call()` ovunque e id verificati da `script.test.ts`; **FE** nessuna modifica e non serviva: la consegna parla dalla porta dei desideri che c'è già, nessun contratto WS nuovo, **nessun bundle da ricostruire**; **`ops/jobs`** nessuna modifica (il backup scopre le tabelle dallo schema, e le bilaterali entrano nel backup di entrambe — dichiarato in ADR-099). Test: 11 unit del parser + 3 della guardia + 11 RLS in `@ugo/db` + 15 d'integrazione in soul + **6 sotto `ugo_app`**. **Nota di sessione**: sandbox senza Docker — `startPostgres()` ha guadagnato il ripiego dichiarato `UGO_TEST_PG_URL` (Postgres 16+pgvector locale, un database usa-e-getta per chiamata; senza la variabile, Testcontainers come sempre), e i due test di `@ugo/db` che istanziavano il container a mano sono passati alla factory | I nonni non sono «i vicini»: una busta con mittente e francobollo non è una federazione — ma sotto RLS anche una busta ha bisogno di una porta |
...
| **Il gruppo 17 chiuso: searxng deployabile, e la procedura per il prossimo container** — riverificato prima di scrivere: **due voci su quattro erano già vere nel compose** (`UGO_RECOGNITION_URL` su soul e `ANTHROPIC_API_KEY` sui job, cablate e mai spuntate). Restavano searxng — **zero righe nel runbook**, quindi la finestra sul mondo di ADR-063 era irraggiungibile in produzione — e la procedura riusabile. Ora: **§2.3-ter searxng** (immagine pubblica, nessuna porta, le tre capabilities che l'immagine pretende, la prova con `format=json`, e soprattutto il `SEARXNG_URL` su soul **senza cui il container gira e non lo chiama nessuno**); **§2-ter «onboarda un container nuovo»** in nove punti, col cablaggio su soul messo al quinto posto perché è la riga che si dimentica sempre — è successo con la percezione E con searxng, entrambe costruite e mai raggiunte; giro di fumo aggiornato (`/health` ora nomina `perception`, e due prove nuove per le due finestre); voce di troubleshooting **«il container c'è, ma non lo usa nessuno»** col modo di distinguere in dieci secondi variabile mancante / container giù / tutto a posto. Giro regola 12: **BO** nessuna modifica (il codice c'era già); **`/admin`** nessuna; **FE** nessuna — questo gruppo era **documentazione operativa**, ed è esattamente ciò che mancava | Un container acceso che nessuno chiama è indistinguibile da un container spento |
| **Le cose che nessuno guardava (ADR-101)** — Fase D, terzo lotto: quattro voci, un tema — **cose che il sistema sa e non dice**. Le **sei cause** della psiche che arrivavano in inglese (`reward`, `used_prop`, `napped`, `loud_noise_muffled`, `calm_voice`, `excited_voice`) hanno la loro parola, con una guardia che confronta le chiavi di `EVENT_PERTURBATIONS` col SOLO blocco `CAUSE_LABEL` — cercare in tutto il modulo avrebbe contato per buone le chiavi di qualunque altra mappa. **`/health` guarda la percezione**: `off` quando non è configurata (una casa senza riconoscimento è una scelta), `error` + `degraded` quando è giù — mai `unavailable`, che senza volto UGO parla ancora. **`GET /v1/jobs/reports`** non ha richiesto righe nuove: i marcatori `dream_step_completed` c'erano già, serviva guardarli **l'ultima volta per passo** invece delle ultime duecento righe. **`GET /v1/kiosks`** espone la mappa che `SceneHub` teneva in RAM (limite dichiarato: con due repliche direbbe solo quelle del processo che risponde). E le **tre rotte senza consumatore**: `graph/size` cablata, `memories/search` **dichiarata** (la usa la CLI — «senza consumatore» era falso, mancava la frase), `POST /v1/beings/:id/enroll/voice` col presign **TOLTA** — nessun consumatore, nessun test, e accettava un riferimento a un oggetto attribuendolo a una persona: non è codice che si tiene per ogni evenienza quando tocca la biometria. **Con questo il gruppo 14 è chiuso**: 14 voci vere, 14 fatte (più 2 che erano già chiuse e la lista non lo sapeva). Giro regola 12: **BO** health, journal, sceneHub, rimozione; **`/admin`** blocco «cosa gira di notte, e chi è collegato» + le sei parole; **FE** nessuna modifica | Una porta senza consumatori e senza test, che attribuisce un file a una persona, non si tiene «per ogni evenienza» |
| **Le chiavi, le correzioni e la mela (ADR-100)** — Fase D, secondo lotto del gruppo 14: quattro atti che si compivano **solo entrando nel container**. `GET/POST/DELETE /v1/tokens` (il valore in chiaro una volta sola; revocare non cancella la riga — sapere che c'era vale quanto sapere che non vale più; **revocare la PROPRIA chiave è concesso**, perché chi ha perso il telefono apre il pannello da un altro dispositivo e impedirlo proteggerebbe la sessione invece della casa); `GET/DELETE /v1/corrections` — e una correzione **si cancella**, a differenza di un ricordo: non è una cosa che UGO ha pensato, è un'istruzione che gli abbiamo dato, e la propria parola si ritira; `PATCH /v1/rooms/:id` che rinomina **portandosi dietro chi ci vive** (senza il secondo UPDATE su `location_label` il rename era una cancellazione travestita); `POST /v1/gosini/:id/reward` con l'`act` della riga — il campo era nel contratto da ADR-058 e arrivava sempre vuoto, quindi `act_efficacy` riceveva «bravo in generale», che non insegna quale gesto è piaciuto. Il test della revoca ha richiesto di **chiudere la porta nel telaio** (`internalToken`): senza, il server è aperto per lo sviluppo e ogni token ignoto diventa operatore — «la chiave revocata non apre più» sarebbe stato un test che non prova niente. Trappola documentata e non testata: un backtick non escapato in un modulo del pannello chiude il template (lo prende `tsc` in un secondo; una guardia non lo distinguerebbe dai backtick escapati legittimi). Giro regola 12: **BO** 8 rotte + verbo `token_revoked`; **`/admin`** due blocchi nel Giornale + la 🍎 sull'iniziativa (listener armato una volta sola, o alla quinta apertura una mela ne dava cinque); **FE** nessuna modifica. Test: 7 d'integrazione | Una chiave che non si può revocare da dove ti accorgi del problema è una chiave che non si può cambiare |
| **Il giornale e la cronaca (ADR-102)** — Fase D, primo lotto del gruppo 14. Riverificata la lista (è del 2026-08-16): **due voci erano già chiuse** — `GET /v1/beings`, che il pannello non chiama più, e il diario, aperto da ADR-079 — e quattordici erano vere. Le tre peggiori erano la stessa famiglia: dati scritti da mesi che **nessuno poteva guardare**. Ora `GET /v1/audit` (il registro di ADR-049, con le sue sette colonne e **nemmeno una in più** — un test asserisce l'elenco esatto, così un campo aggiunto per comodità diventa rosso), `GET /v1/messages` (le conversazioni di casa **in chiaro**, come le ha dette il proprietario: il cliente della reception le aveva, lui vedeva un contatore) e `GET /v1/perception` (chi il riconoscitore ha creduto di vedere e quanto ci credeva — **mai l'embedding**, che resta cifrato dov'è). Tutte e tre `inAccount` e admin: è la biografia della casa. Cursore `?prima=` invece della pagina intera: dodici mesi di registro non entrano in una schermata, e caricarli tutti è il modo di non guardarlo mai. Pannello: UNA pagina «Il giornale» coi tre blocchi, perché sono la stessa domanda guardata da tre lati. Giro regola 12: **BO** tre rotte; **`/admin`** pagina, script, stile e voce nel menu; **FE** nessuna modifica — il muso non deve sfogliare il registro di casa. Test: 7 d'integrazione su due case (il vicino non compare, le chiavi sono esattamente quelle, 403 per chi non amministra), morso verificato togliendo lo scope | Un registro che il titolare non può guardare non è un registro: è un file |
...
| ~~RLS è presente e inerte in produzione~~ (ADR-048/062) | — | **Conversione COMPLETA (2026-08-19, PR #87–#91)**: 27 file di rotte su `inAccount`, il mercato sul ruolo `ugo_market` (ADR-097), i runtime sulla connessione della casa (ADR-098), la reception col token che si risolve fuori e il cliente dentro (0050), i job Python che dichiarano la casa per ogni giro. Il censimento è a **25 casi** su server intero con `ugo_app` vero. **Accenderla in produzione è runbook §8-bis** (password di `ugo_app` + `DATABASE_URL_APP` + giro di fumo), reversibile togliendo la variabile — la mossa giusta è farlo alla reinstallazione da zero, col muro acceso dal primo giorno |
