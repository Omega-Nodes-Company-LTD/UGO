...
| **Il verso ha un timbro (ADR-116)** — ultima voce di «fenotipo dal genoma», e non era una mappa genoma→aspetto come le altre: il grugnito **non esisteva come suono** (`grunt` era un gesto visivo — bocca, orecchie, una scossa), quindi dargli un timbro voleva dire aprire un **canale sensoriale nuovo**. Timbro da `chonk` e `snout`: **nessun gene nuovo, zero migrazioni** — un maiale grosso suona più basso, un grugno lungo più scuro, che è la fisica di una cassa di risonanza e non un'impostazione; deterministico come le chiazze. **Il grugnito non è la voce**: mentre parla non grugnisce, perché un verso sopra una frase è una frase che non si capisce. Le tre regole del silenzio sono quelle della pioggia e **non nuove** — parte solo dopo il gesto dell'utente, di notte tace, e l'interruttore è uno solo (due interruttori per il suono sono il modo in cui uno resta acceso per sbaglio). Sintesi WebAudio, zero asset. Il roster porta i tratti fino al chiosco, o tutti i gosini di una stanza suonerebbero uguali. **Resta fuori il repertorio**, che è «genoma strutturale»: aggiungere tre versi a caso sarebbe contenuto travestito da codice | Un tratto che non si sente non è ereditabile: la genetica finisce dove finiscono i sensi |
| **La reception è una nostra stanza (ADR-115)** — il porcello 3D viveva dentro `apps/face`, cioè il **chiosco**: la reception mostrava al cliente un muso 2D disegnato a mano, onesto e **di un altro animale** rispetto a quello che vive in casa. Ora `@ugo/face-body` (stessi file, spostati; `main.ts`, sensori, microfono e WS **restano** nel chiosco, perché un package condiviso che apre un microfono è un package che nessuno monta). Tre ripieghi sulla stessa linea — non rompere la pagina per una decorazione: import **dinamico e solo nel browser** (è three.js, e Next lo renderizzerebbe dove un canvas non esiste), **ripiego sul muso 2D** se non parte (chi non vede niente pensa che sia rotto il servizio, non la scheda video), `look` **facoltativo** (senza, il corpo medio). `/v1/reception/me` porta gli **otto geni del corpo e nient'altro** — regola della vetrina (ADR-083) con un motivo in più: il cliente non è di casa, e quanto è affettuoso il gosino di qualcun altro non sono affari suoi. L'**umore viaggia attaccato alla risposta**, non su un canale: un WS darebbe al cliente una finestra su come sta una creatura anche quando non le parla; e accompagna **ogni** risposta, cache e ticket compresi, perché se accompagnasse solo quelle a pagamento il corpo si fermerebbe alla domanda ripetuta e sembrerebbe un guasto invece di una cache che funziona. **L'estrazione ha rotto un test nel modo giusto**: lo scanner degli id dei gesti guardava la cartella sopra la sua, quindi vedeva tutto il chiosco — spostato, ha smesso di vedere `main.ts`, e la copertura si sarebbe persa in silenzio se non avesse avuto una guardia sul **numero** di chiamate trovate | Un test che si accontenta di non trovare niente non è una rete: la guardia sul numero è ciò che ha trasformato una perdita silenziosa in un rosso |
| **Lo screening in silico non filtra niente (ADR-114)** — costruita la giornata d'oro e misurata su **243 caratteri raggiungibili** (la griglia completa dei limiti che `characterFrom` impone): **bocciati zero**. Non è una soglia da tarare, è **strutturale** — nel motore la risposta agli eventi è uguale per tutti, e del genoma dipendono solo i livelli di riposo, già dentro limiti sani per costruzione. Quindi **nessun filtro alla nascita**: sarebbe una simulazione pagata per ogni cucciolo per un controllo che passa sempre, e un controllo che non ha mai detto di no non protegge — rassicura, che è peggio. La giornata **resta come guardia sui limiti**: diventa rossa se qualcuno allarga i clamp o se il motore smette di riportare indietro una variabile spinta. E la prima versione era sbagliata in modo istruttivo, smontata dalla prova al primo giro: bocciava **il gosino più curioso che il sistema può produrre** (baseline 0,95, il tetto), cioè buttava il diverso invece del rotto — la gara che il proprietario aveva escluso. La regola giusta distingue **inchiodata** (a un estremo e non si muove più) da **intensa** (sta in alto, si muove, torna) | Un risultato negativo misurato vale più di un meccanismo plausibile: il secondo si sarebbe pagato a ogni cucciolo per non dire mai niente |
| **I luoghi dell'account (ADR-113)** — ADR-092 aveva separato il titolare dal luogo e si era fermata a metà: sulla riga dell'account restavano `place`/`lat`/`lon`, cioè **una geografia sola**, e chi ha la casa in città e quella al mare aveva un cielo solo — sbagliato per forza in una delle due. `places` per account + `rooms.place_id`; le colonne vecchie **si tolgono**, non si affiancano (due verità sulla stessa cosa sono il difetto che ADR-092 è servita a togliere) e la migrazione **traghetta**: ogni account riceve il suo primo luogo col nome che aveva, o «Casa», e tutte le sue stanze ci finiscono dentro. Il **fuso resta dell'account**, ed è una scelta: una notte appartiene a chi vive, non alle stanze. Un luogo abitato **non si butta** (409 con quante stanze ha): la cascata butterebbe la piantina per un click, staccarle le lascerebbe senza cielo. Il meteo segue la **stanza**. `GET /v1/places?q=` (ricerca nel mondo) diventa `/v1/places/search`: erano già due risorse sullo stesso indirizzo. **Due difetti trovati dai test d'integrazione, non dalla rilettura**: le rotte erano registrate dentro il blocco delle creature — una casa senza runtime accesi rispondeva 404 al proprio indirizzo — e un account **nuovo** nasceva senza luoghi, perché il traghetto riguardava solo il passato | Un traghetto che guarda solo indietro lascia scoperto il futuro; e una rotta registrata sotto la condizione sbagliata è una rotta che a volte non c'è |
| **Un gioco è fatto di turni (ADR-112)** — la voce era rimasta aperta con la ragione scritta: fra un turno e l'altro qualcosa deve ricordare *cosa è stato pensato e cosa non è ancora stato detto*, e in questo sistema **non esisteva nessuno stato di turno**. Non può essere la cronaca della conversazione, per due ragioni e la seconda è quella grave: il segreto sarebbe leggibile insieme al resto, e finirebbe **nel prompt del turno dopo** — cioè consegnato al modello che dovrebbe custodirlo. Riga sua, numero **cifrato a riposo**, e a giudicare è un confronto fra due interi: **zero token**. `randomInt` e non `Math.random` (uno indovinabile conoscendo l'ora d'inizio non è un numero pensato). Le decisioni piccole, che sono quelle che si sbagliano: una partita aperta per esemplare tenuta da un **indice unico parziale** e non da un controllo applicativo; `playing` **cambia il senso della frase** (fuori partita un numero non è un tentativo, o il gioco comincerebbe da solo a ogni età detta); scade dopo 12 ore **leggendo**, perché un numero pensato a marzo trasformerebbe ogni cifra di giugno in un tentativo; la resa si **offre** e non si impone; solo in casa. E l'export esce col numero in chiaro: esportare a partita aperta è uno spoiler, e nasconderlo sarebbe la prima bugia di un export che promette tutto | Uno stato che non esiste non si inventa di sfuggita dentro un altro lavoro: è così che nascono i segreti scritti nel posto sbagliato |
| **I documenti di casa (ADR-111)** — «UGO conosce solo ciò che ha sentito»: il libretto della caldaia esiste, e per lui no finché non glielo si dà da leggere. La macchina era **già scritta e provata** (bucket, chiave opaca, nome cifrato, estrazione, finestre, embedding locale, ciphertext, ricerca vettoriale) ma viveva tutta dentro la reception, per i clienti. **Non si è allargata `customer_chunks`**, che era la strada più corta: là lo scope è `customer_id` e la reception ci legge sopra, quindi un referto del veterinario starebbe **a un `where` di distanza** dall'occhio di un cliente. Il file **non passa da soul** (URL firmato, dritto nel bucket) e si **legge di notte**: un PDF lungo sono centinaia di embedding e nessuno deve restare col dito sul bottone. Soglia 0,5 contro lo 0,35 dei ricordi, ed entrano **col nome del file** — un ricordo si ridice con parole sue, un documento si **cita**, perché citare un documento suona autorevole anche quando è sbagliato. **L'export si è rotto da solo, ed è la cosa giusta**: il test di copertura di ADR-089 è diventato rosso appena le tabelle sono esistite | La differenza fra un ricordo e un documento è chi lo firma: il primo lo dici con parole tue, il secondo lo citi |
| **La ricerca contava meno dell'anagrafica (ADR-110)** — ultima voce del gruppo 1, e la diagnosi del backlog era vicina ma non esatta. Misurata l'escursione dei tre fattori sui primi cinque: `relevance` **2,1×** (e quasi tutta dall'**accordo** dei bracci — fra la prima e la quinta posizione dentro lo stesso braccio c'è il **6%**), `importance` **3,0×**, `recency` **1,9×**. Quindi `importanza × recency` fa **3,2×** e **scavalca la ricerca**. E la metà che mancava: l'importanza degli **episodi** è sistematicamente più bassa di quella dei fatti perché così li scrive chi li estrae — un episodio nasce meno importante **e** invecchia più in fretta, e la formula glielo faceva pagare **due volte**. Correzione minima sulla causa: i due termini diventano **modulatori in banda**, l'ordine a parità di relevance non cambia di una posizione. **E la CI ha smentito la seconda metà**: a «cosa si è rotto in casa?» l'episodio della lavatrice **non viene ripescato affatto** (`lexHits: 0`) — per quella domanda non è la formula ma **l'indice**, e nessun riordino può ripescare ciò che non è fra i candidati. Floor a 0 per registrare la misura invece di asserire una cosa che il recupero non fa, e una voce nuova e onesta nel gruppo 1 | Una diagnosi plausibile si misura prima di correggerla, e poi si rimisura dopo: due volte di fila la causa non era quella che sembrava |
| **Un gene nuovo non azzera i pedigree (ADR-109)** — la voce di backlog era «mancano le setole», un gene e una cresta. A guardare **cosa succede a un genoma già scritto quando il catalogo cambia** è saltato fuori il difetto peggiore della giornata, e il peggiore perché **silenzioso**: `genomeFromStoredTraits` accettava la mappa degli alleli solo se c'erano TUTTI i geni del catalogo corrente (`GENE_KEYS.every`), altrimenti la buttava intera e ricostruiva il genoma **omozigote dai valori espressi** — cioè da quello che si vede addosso. Finché il catalogo non cambia non succede niente, e infatti non era mai successo; ma il catalogo **è cambiato tre volte** (ADR-068 due geni, ADR-071 uno), e ogni aggiunta ha reso di colpo incompleto ogni genoma precedente. Un esemplare che porta uno `spots` alto **coperto** — il portatore che ADR-068 ha progettato apposta, la ragione per cui da due genitori senza chiazze nasce ogni tanto un cucciolo a chiazze — veniva riletto omozigote sul valore basso: l'allele nascosto spariva, e alla prima riscrittura di `trait_sets` spariva per sempre. Nessun errore e nessun test rosso, solo una stirpe che smette di dare chiazze e un allevatore che non capisce perché. Il test che lo dimostra è di tre righe ed era **rosso**: `[0.1, 0.9]` tornava `[0.1, 0.1]`. Ora **locus per locus**: basta un allele valido perché la mappa conti come genotipo, ogni locus valido si tiene com'è (l'eterozigosi non si perde per colpa di un vicino), e i mancanti o malformati si riempiono omozigoti preferendo il **valore espresso** conservato accanto — così nessuno cambia aspetto perché è stato aggiunto un gene altrove — e solo in mancanza il default del catalogo. Intatta la strada pre-ADR-068 (nessun `alleles` ⇒ scalari, e lì l'omozigosi è giusta perché non c'è genotipo da perdere). **Zero migrazioni**: i genomi vecchi restano dove sono e si leggono meglio di prima, che è anche l'unica strada compatibile con l'immutabilità di `trait_sets` (ADR-015). Primo utente del meccanismo, le **setole**: `bristle` blend 0.35, visibili da 0.30 — sotto, dorso liscio, e un maiale glabro è un fenotipo non un errore; il gene muove **quante e quanto lunghe** insieme (un dorso irto non è lo stesso pelo più fitto); posizioni in `BRISTLE_SITES` fisse come `SPOT_SITES`, perché ciò che non è stabile non è un tratto ma un effetto; colore da `aged()`, quindi **sbiadiscono con l'età** — cosa che il commento di `greying` e `/documentation` promettevano da ADR-071 mentre di setole da sbiadire non ce n'era una. In vetrina sì (aspetto, ADR-083), temperamento mai. Giro regola 12: **BO** psyche + il lettore + `character.ts` + `vetrinaService`; **`/admin`** l'etichetta «setole» — la pagina «Com'è fatto» elenca i geni dal catalogo e il gene nuovo comparirebbe da solo, ma **col nome inglese**, che è il modo in cui un pannello comincia a parlare la lingua del database; **FE** la cresta in `pig.ts`, contratto invariato (i tratti viaggiano come `Record<string, number>`) ma **bundle del muso da ricostruire al deploy**, o sui dispositivi le setole non spuntano. Python non toccato **e non serviva**: `hygiene.py` legge per chiave con un default ed era già tollerante, col suo test «un genoma senza longevity invecchia lo stesso» | Un difetto che non fa rumore aspetta il cambiamento che lo attiva: questo era armato da tre ADR, e sarebbe scattato al quarto |
