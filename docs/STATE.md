...
| **…ma tre scrittori cifrano lo stesso, e il risultato è due difetti diversi** (trovato con ADR-086) | **Alto, e silenzioso.** Lascito (ADR-075), lezioni dell'anziano (ADR-077) e dote (ADR-074) scrivono `memories.text` **cifrato**, contraddicendo la riga qui sopra — e i due bracci di ADR-022 reagiscono in modo opposto. **(a)** Lascito e lezioni non hanno embedding (chi le scrive non ha un embedder) e il braccio lessicale gira sul `search_vector` del ciphertext: **non si ripescano mai**, quindi la promessa di ADR-075 «il lascito resta» è vera sul database e falsa nell'uso. **(b)** La dote invece l'embedding ce l'ha, calcolato sul **testo in chiaro** prima di cifrarlo: quelle righe si ripescano benissimo e **il ciphertext finisce nel prompt**, sotto «Ricordi pertinenti», dove UGO proverà a parlarne. Il secondo è peggio del primo: non è memoria che manca, è spazzatura che arriva | Non aggiustabile leggendo: è la decisione di sopra da riaprire davvero. Tre strade — embedding prima di cifrare **e decifratura nel recupero** (si tiene solo il braccio vettoriale); testo in chiaro anche lì, coerente con ADR-022, affidandosi alla cifratura del volume; cifratura ricercabile. Vuole il suo ADR e una misura su una memoria vera |
