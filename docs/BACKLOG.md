...
| 🔨 | **UGO deve poter dire «non lo so»** | **misurato, e il criterio relativo NON regge (ADR-106)**. `gap`, `gapRel` e `plateau` non separano male: separano **al contrario** — «quando è il compleanno della nonna?», che una risposta ce l'ha, ha `plateau` −0.0504, più negativo di tutte e tre le domande senza risposta. `bothArms`/`lexHits` sono segnali **positivi a senso unico** (zero falsi positivi, ma «no» non prova niente: 4 risposte vere su 10 ce l'hanno falso). `top1` sembrava separare per **28 decimillesimi** — lo stesso margine tarato dei 0.675 che ADR-022 aveva già rifiutato. E il banco non poteva accorgersene: aveva **tre** domande senza risposta, tutte lontane dal dominio, quindi nessuna poteva falsificare `bothArms`. Ora sono **dieci**, con sette near-miss dentro il dominio (il nome del cane del vicino, il colore della macchina, la password del wifi Cinghiale che aggancia il braccio lessicale). **Restano le due strade vere**: la verifica del modello locale sui ricordi ripescati (candidata del prossimo ADR, mai il provider — regola 3) o un embedder che separi meglio |
