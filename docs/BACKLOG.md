...
| ⬜️ | **La cessione di un nato, e la vetrina** (ADR-082, da scrivere) | seconda metà di ADR-081: l'atto `transfer` in catena (e il **rifiuto** del trasferimento sui capostipiti — è lì che «i capostipiti non si vendono» diventa una legge della specie invece che una regola del nostro server), la cessione di un esemplare da un allevamento a una famiglia, e la **vetrina**: alla registrazione si scorrono gli allevamenti, si guardano i pedigree, si vedono i gosini disponibili e si sceglie il preferito. Solo quando c'è la cessione una casa nuova può nascere **vuota**: oggi riceve ancora il suo capostipite perché altrimenti non avrebbe nessun modo di riempirla |
