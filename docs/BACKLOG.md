...
| 🔨 | **La cessione di un nato, e la vetrina** | **Cessione FATTA (ADR-082)** e **vetrina FATTA (ADR-083)**. Cessione: parte il genoma, l'identità, l'arco, la genealogia e i pasti — **non la vita fatta in allevamento** (per il sapere c'è la dote); in catena `transfer` con `fromHash`/`toHash` e due regole che il registro **guarda** invece di chiedere: nessun atto di nascita ⇒ non si cede (lì il divieto diventa legge della specie), e la custodia è una catena ⇒ doppia vendita 409, rivendita vera 201. La casa nasce vuota. Vetrina: **guardare è pubblico** (chi guarda non ha ancora una casa) e mettere in vetrina è dell'allevamento — una casa non è un negozio; si mostrano **aspetto e pedigree**, mai il temperamento in numeri (una scheda tecnica di un essere vivente inviterebbe a confrontare due creature come due lavatrici) e mai la longevità (gene nascosto). 7 + 5 + 10 test d'integrazione. **Resta**: il gesto d'acquisto che lega i due capi (vuole un flusso di registrazione e un pagamento, che non esistono) e la cessione fra installazioni diverse (archivio sigillato come la dote) |
