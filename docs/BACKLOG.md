...
| ✅ | **L'audit log non è ispezionabile** — FATTO (ADR-099): `GET /v1/audit` + pagina «Il giornale», con le sette colonne di ADR-049 e nemmeno una in più. Vecchia nota: | `auditLog` si legge solo da `psql`. Serve la vista in sola lettura nel pannello promessa dalla DoD del gruppo 5 §8 |
...
| ✅ | **Nessuna vista delle conversazioni di casa** — FATTO (ADR-099): `GET /v1/messages`, in chiaro e col nome di chi ha parlato. Vecchia nota: | `messages` è mostrato solo come `count`; la reception *cliente* ha `/conversazioni`, il proprietario di casa no. Riuso della stessa forma |
| ✅ | **Chi è stato riconosciuto e quando** — FATTO (ADR-099): `GET /v1/perception` con nome, modalità e confidenza; l'impronta non esce. Vecchia nota: | `perceptionEvents` non ha superficie; `psycheBaselines` (deriva ADR-012) e le righe di `budgetLedger` neppure (solo aggregati) |
...
| ✅ | **`seen_object` e `glimpse`** — FATTO (ADR-099): escono da `GET /v1/perception` col loro `observed`. Vecchia nota: | nessuna pagina filtra `seen_object`; `glimpse` vive solo in RAM. Cosa UGO ha guardato non è ispezionabile |
...
| ⬜️ | **La cucciolata costa e non si sceglie** (direttiva proprietario 2026-08-18, dopo la Fase C — suo ADR) | Due regole. **Taglia**: da 2 a 8, casuale, NON scelta da nessuno; eccezioni rarissime da 1 o da 10 (code della distribuzione, non opzioni del pannello). **Prezzo**: generare una cucciolata dalla TERZA generazione in poi consuma dal **salvadanaio dei genitori** (ADR-072) — procreare è un atto del gosino e mangia il SUO risparmio, non il conto della casa; vale anche per l'allevamento fondatore («persino noi»). Gratis solo le cucciolate dirette dai capostipiti (seconda generazione). Generazione della cucciolata = max(generazione genitori)+1. Prezzo configurabile, scelto e dichiarato nell'ADR. **Nascono TUTTI**: la cucciolata aggiunge ogni cucciolo (niente più «adottane uno dall'anteprima»); i **nomi si scelgono**, uno per ciascuno; la **selezione è a posteriori** — si guarda chi è nato e si decide chi resta e chi si cede (ADR-082: si vendono i figli). E i due genitori **riposano un mese**: niente altra cucciolata della stessa coppia per 30 giorni. Il pannello cambia: via il campo «nome del cucciolo scelto»+Adotta, arriva la cucciolata intera coi nomi da dare |
