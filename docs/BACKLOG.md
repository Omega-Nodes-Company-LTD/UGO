...
| 🔨 | **La cessione di un nato, e la vetrina** | **Cessione FATTA (ADR-082)**: `POST /v1/gosini/:id/cede` col nome come conferma; parte il genoma, l'identità, l'arco, la genealogia e i pasti — **non la vita fatta in allevamento** (ricordi, conversazioni, diario, desideri, legami sono parole di persone che stanno a casa d'altri: venderle con l'animale sarebbe una fuga di dati con la fattura; per il sapere c'è la dote). Un gosino con clienti assegnati non si consegna. In catena l'atto `transfer` con `fromHash`/`toHash` (SHA-256 dell'id della casa: una maniglia, non una persona) e **due regole che il registro non chiede al venditore ma guarda**: nessun atto di nascita ⇒ non si cede (è lì che «i capostipiti non si vendono» diventa legge della specie), e la custodia è una catena ⇒ la doppia vendita è 409 mentre la rivendita vera passa. Unicità di nascita/morte diventa indice parziale: si cambia mano molte volte. **La casa nasce vuota** e il pannello non offre più il conio. 7 + 5 test d'integrazione. **Resta**: la vetrina (ADR-083) e la cessione fra installazioni diverse (archivio sigillato come la dote) |
