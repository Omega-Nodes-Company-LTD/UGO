...
| | Punto | Note |
|---|---|---|
| 🔨 | **Cucciolate: ricombinazione del genoma** (orizz. 1) | **Motore FATTO (ADR-068, 2026-08-17)**: genoma diploide come superset del jsonb esistente (fondatori omozigoti, zero migrazioni, lettori intatti), ceppi (8, non sessi), dominanza/epistasi (`spots` recessivo: la rarità emerge), poliparentale a pesi, anello 0.04–0.55, screening binario che filtra i rotti, PRNG iniettato (`mulberry32`) — `packages/psyche/src/genes.ts` + `genetics.ts`, 21 unit test su funzioni pure (regola 1). **Resta**: la nascita nel database (riga `gosini` + `trait_sets` v1 + provisioning), il trigger dall'incontro BLE (gruppo 6), `spots`/`tail` disegnati dal muso |
