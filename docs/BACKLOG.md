...
| ⬜️ | **I ricordi cifrati: due difetti opposti** | **Trovato con ADR-086, dichiarato e non risolto lì** perché non è un difetto di lettura ma una decisione di architettura. Lascito (ADR-075), lezioni (ADR-077) e dote (ADR-074) scrivono `memories.text` cifrato, contro la scelta dichiarata in ADR-022 di tenerlo in chiaro. **(a)** Lascito e lezioni non hanno embedding e il braccio lessicale gira sul ciphertext: **non si ripescano mai** — «il lascito resta» è vero sul database e falso nell'uso. **(b)** La dote l'embedding ce l'ha, calcolato sul testo **in chiaro** prima di cifrare: si ripesca, e **il ciphertext finisce nel prompt** sotto «Ricordi pertinenti». Il secondo è peggio: non è memoria che manca, è spazzatura che arriva. Tre strade — embedding prima di cifrare **più decifratura nel recupero**; testo in chiaro anche lì; cifratura ricercabile → vuole il suo ADR |
