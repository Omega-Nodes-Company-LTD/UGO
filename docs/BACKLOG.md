...
| 🔨 | **RLS con ruolo Postgres dedicato** | **ADR-046**, tempo 1 fatto: ruolo `ugo_app`, politiche su tutte e 22 le tabelle, `withHousehold()` con `SET LOCAL`. Senza `FORCE`, quindi in produzione **inerte** finché non entra il tempo 2 |
| ⬜️ | **Caduta dei `DEFAULT`** su `gosino_id` e `household_id` | tempo 2 di ADR-046. Prima serve che `GosinoRegistry` carichi una casa per volta dentro `withHousehold`, o come `ugo_app` non vedrà nessun esemplare (ADR-046 §7) |
...
| ⬜️ | **Audit log** | fondamenta per qualunque discorso di conformità. Deciso: 12 mesi, solo ID e verbi, append-only imposto dai `GRANT` (ADR-047 da scrivere) |
