...
| 🔨 | **Export e oblio self-service dal muso** | **Primo tempo FATTO (ADR-089): il diritto era incompleto**, e portarlo sul chiosco così avrebbe voluto dire prometterlo a più gente. `exportService.ts` prometteva «ogni byte» e ne saltava **diciassette tabelle**: chi è stato visto e quando (`perception_events`), quante volte è passato uno sconosciuto, la piantina di casa e come è arredata, la spesa, le domande che UGO ha imparato a fare («se ho preso le medicine»), il genoma delle creature, gli atti di nascita e le adozioni. Ora escono tutte, con due esclusioni **dentro** le tabelle: **nessun vettore biometrico** (esce il fatto che qualcuno è passato, non il suo volto — e non è filtrato dopo, non è mai stato selezionato) e **nessuna credenziale** (`access_tokens` è l'unica voce dell'elenco «non personale», con scritto perché). Il decifratore ora tollera i due mondi, quindi ricordi e diario escono **leggibili** invece che in base64. E un test legge schema ed export e pretende che ogni tabella sia in uno dei due elenchi: l'elenco a mano è giusto — su una tabella che porta biometria la differenza fra le colonne è tutta — ma invecchia in silenzio, e adesso parla. 46 unit + 7 d'integrazione. **Secondo tempo: il chiosco** |
