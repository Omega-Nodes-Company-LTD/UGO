...
| 🔨 | **UGO deve poter dire «non lo so»** | *misurato in ADR-022*: le bande di similarità di domande con e senza risposta si sovrappongono (0.624–0.893 contro 0.604–0.672). Nessuna soglia assoluta le separa: serve un criterio **relativo**, una verifica del modello, o un embedder migliore. **In corso, e nell'ordine giusto**: il banco stampa adesso i segnali candidati per ogni domanda del corpus — `gap` fra primo e secondo, `gapRel`, `plateau` sulla media degli altri, `bothArms` (il primo trovato da entrambi i bracci), `lexHits` — separati fra domande con risposta e senza. Non asserisce niente e non cambia comportamento: **la misura viene prima della scelta**, perché scegliere il criterio senza vedere i numeri vuol dire tararlo sul corpus, cioè aver visto le risposte (l'errore che BASELINE.md documenta). I numeri si leggono dai log della CI — l'embedder vero gira solo lì |
