...
| ⬜️ | **Banco evolutivo in silico** (orizz. 1, orologio 1) | generazioni simulate su `packages/psyche` + fixture golden day (Fase 3): giornate sintetiche → misura convergenza → selezione → ricombinazione, migliaia di giri a notte, zero token; è il ciclo di R&D dell'allevamento — la selezione va al ritmo del silicio, non delle vite |
